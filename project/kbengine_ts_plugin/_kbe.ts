module Kbe
{
	...
	export class App
	{
		...
		static get app()
		{
			return App._app;    // 如果外部使用者因为访问到undefined出错，表示需要先Create
		}

		static Create(args: Args): App
		{
			if(App._app != undefined)
			{
				throw Error("App must be singleton.");
			}
			new App(args);
			return App._app;
		}
		...
		static Destroy()
		{
			...
			if(App.app.idInterval)
			{
				clearInterval(App.app.idInterval);
			}

			App.app.UninstallEvents();
			App.app.Reset();
			App._app = undefined;
		}
		...
		{
			Logger.Assert(App._app === undefined, "App::constructor:singleton App._app must be undefined.");
			App._app = this;
		...
		InstallEvents(): void
		{
			Logger.Debug("App::InstallEvents");
			Event.Register("createAccount", this, this.CreateAccount);
			Event.Register("login", this, this.Login);
			Event.Register("reloginBaseapp", this, this.ReloginBaseapp);
			Event.Register("bindAccountEmail", this, this.BindAccountEmail);
			Event.Register("newPassword", this, this.NewPassword);

			Event.Register("onDisconnected", this, this.OnDisconnected);
			Event.Register("onNetworkError", this, this.OnNetworkError);
		}
		...
		{
			Logger.Error("App::OnNetworkError:%s.", event.toString())
			this.networkInterface.Close();
		}

		UninstallEvents()
		{
			Event.DeregisterObject(this);
		}
		...
		private GetBaseappAddr(): string
		{
			...
			if(this.useWss)
			{
				...
				// addr = this.protocol + "127.0.0.1"+ ":" + this.wssBaseappPort + "/baseapp" + "?port=" + this.baseappPort;
			}
			else
			{
				addr = this.protocol + this.baseappAddress + ":" + this.baseappPort;
				// addr = this.protocol + "127.0.0.1" + ":" + this.baseappPort;
			}
			return addr;
		}
		...
		private SendTick()
		{
			...
			{
				//Logger.Debug("App::SendTick...this.networkInterface is not ready.");
				return;
			}
			
			let now = (new Date()).getTime();
			//Logger.Debug("App::SendTick...now(%d), this.lastTickTime(%d), this.lastTickCBTime(%d).", now, this.lastTickTime, this.lastTickCBTime);
			if((now - this.lastTickTime) / 1000 > 15)
			{
				if(this.lastTickCBTime < this.lastTickTime)
				{
					Logger.Error("App::Update: Receive appTick timeout!");
					this.networkInterface.Close();
					return;
				}
		...
		private Login_loginapp(noconnect: boolean): void
		{
			if(noconnect)
			{
				...
				Logger.Info("App::Login_loginapp: start connect to " + addr + "!");
		...
		{
			Logger.Debug("App::onOpenLoginapp_login:success to %s.", this.serverAddress);
			if(!this.networkInterface.IsGood)   // 有可能在连接过程中被关闭
			{
				Logger.Warning("App::onOpenLoginapp_login:network has been closed in connecting!");
				return;
			}
			...
			if(!this.loginappMessageImported)
			{
				...
				bundle.Send(this.networkInterface);
				Logger.Info("App::OnOpenLoginapp_login: start importClientMessages ...");
			}
			else
			{
		...
		Resetpassword_loginapp(noconnect: boolean)
		{
			if(noconnect)
			{
				...
				Logger.Info("App::Resetpassword_loginapp: start connect to %s!", addr);
		...
		{
			Logger.Info("App::onOpenLoginapp_resetpassword: successfully!");
			this.currserver = "loginapp";
			...
			if(!this.loginappMessageImported)
			{
				...
				bundle.NewMessage(Message.messages["Loginapp_importClientMessages"]);
				bundle.Send(this.networkInterface);
				Logger.Info("App::OnOpenLoginapp_resetpassword: start importClientMessages ...");
			}
			else
			{
				this.OnImportClientMessagesCompleted();
		...
		{  
			Event.Fire("onConnectionState", true);
			Logger.Info("App::OnOpenLoginapp_createAccount: successfully!");
			this.currserver = "loginapp";
			...
			if(!this.loginappMessageImported)
			{
				...
				bundle.NewMessage(Message.messages["Loginapp_importClientMessages"]);
				bundle.Send(this.networkInterface);
				Logger.Info("App::OnOpenLoginapp_createAccount: start importClientMessages ...");
		...
		CreateAccount_loginapp(noconnect: boolean)
		{
			...
			{
				let addr = this.GetLoginappAddr();
				Logger.Info("App::CreateAccount_loginapp: start connect to %s!", addr);
		...
		private OnImportClientMessages(stream: MemoryStream): void
		{
			...
			Logger.Debug("App::OnImportClientMessages:import............stream len(%d), msgcount(%d).", stream.Length(), msgcount);
			...
			while(msgcount > 0)
			{
				...
				if(isClientMessage)
				{
					...
					{
						Logger.Error("App::onImportClientMessages[" + App.app.currserver + "]: interface(" + msgname + "/" + msgid + ") no implement!");
					}
				}
				...
				Logger.Debug("App::OnImportClientMessages:import............msgid(%d), msglen(%d), msgname(%s), argtype(%d), argsize(%d).", msgid, msglen, msgname, argtype, argsize);
			}
		...
		{
			Logger.Info("App::onImportClientMessagesCompleted:successfully......currserver(%s) currstate(%s).", this.currserver, this.currstate);
			this.Hello();
			...
			if(this.currserver === "loginapp")
			{
				...
				if(!this.serverErrorsDescrImported)
				{
					...
					Logger.Info("KBEngine::onImportClientMessagesCompleted(): send importServerErrorsDescr!");
			...
			else
			{
				...
				{
					Logger.Info("App::onImportClientMessagesCompleted: start importEntityDef ...");
					let bundle = new Bundle();
		...
		{
			Logger.Info("App::onImportEntityDefCompleted: successfully!");
		...
		private Hello()
		{
			Logger.Info("KBEngine::Hello.........current server:%s.", this.currserver);
			let bundle: Bundle = new Bundle();
			if(this.currserver === "loginapp")
			{
		...
		{
			Logger.Info("KBEngine::Client_onHelloCB.........stream length:%d.", stream.Length());
			...
			Logger.Info("App::Client_onHelloCB: verInfo(" + this.serverVersion + "), scriptVerInfo(" + 
		...
		{
			Logger.Debug("KBEngine::Client_onVersionNotMatch.........stream length:%d.", stream.Length());
			this.serverVersion = stream.ReadString();
			Logger.Error("Client_onVersionNotMatch: verInfo=" + this.clientVersion + " not match(server: " + this.serverVersion + ")");
			Event.Fire("onVersionNotMatch", this.clientVersion, this.serverVersion);
		}

		Client_onScriptVersionNotMatch(stream: MemoryStream)
		{
			this.serverScriptVersion = stream.ReadString();
			Logger.Error("Client_onScriptVersionNotMatch: verInfo=" + this.clientScriptVersion + " not match(server: " + this.serverScriptVersion + ")");
			Event.Fire("onScriptVersionNotMatch", this.clientScriptVersion, this.serverScriptVersion);
		}
		...
		Client_onAppActiveTickCB()
		{
			...
			this.lastTickCBTime = dateObject.getTime();
			Logger.Debug("KBEngine::Client_onAppActiveTickCB.........lastTickCBTime:%d.", this.lastTickCBTime);
		}
		...
		Client_onImportServerErrorsDescr(stream: MemoryStream)
		{
			...
			while(size > 0)
			{
				...
				Logger.Info("Client_onImportServerErrorsDescr: id=" + error.id + ", name=" + error.name + ", descr=" + error.description);
			}
		}
		...
		Client_onLoginFailed(stream: MemoryStream)
		{
			...
			Logger.Error("App::Client_onLoginFailed: failedcode(" + this.serverErrors[failedcode].name + "), datas(" + this.serverdatas.length + ")!");
			Event.Fire("onLoginFailed", failedcode);
		}
		
		Client_onLoginSuccessfully(stream: MemoryStream)
		{
			Logger.Debug("Client_onLoginSuccessfully------------------->>>");
		...
		private Login_baseapp(noconnect: boolean)
		{
			if(noconnect)
			{
				...
				Logger.Info("App::Login_baseapp: start connect to " + addr + "!");
		...
		{
			Logger.Info("App::onOpenBaseapp: successfully!");
			this.currserver = "baseapp";
		...
		{
			Logger.Error("App::Client_onLoginBaseappFailed: failedcode(" + this.serverErrors[failedcode].name + ")!");
			Event.Fire("onLoginBaseappFailed", failedcode);
		}

		Client_onReloginBaseappFailed(failedcode)
		{
			Logger.Error("App::Client_onReloginBaseappFailed: failedcode(" + this.serverErrors[failedcode].name + ")!");
		...
		Client_onReloginBaseappSuccessfully(stream: MemoryStream)
		{
			...
			Logger.Debug("App::Client_onReloginBaseappSuccessfully: " + this.userName);
		...
		OnImportClientEntityDef(stream: MemoryStream)
		{
			...
			while(!stream.ReadEOF())
			{
				...
				Logger.Info("App::OnImportClientEntityDef: import(" + scriptmodule_name + "), propertys(" + propertysize + "), " +
						"clientMethods(" + methodsize + "), baseMethods(" + base_methodsize + "), cellMethods(" + cell_methodsize + ")!");

				let module:ScriptModule = new ScriptModule(scriptmodule_name);
				if(module.script === undefined)
					Logger.Error("App::OnImportClientEntityDef: module(" + scriptmodule_name + ") not found!");

				module.name = scriptmodule_name;
				MODULE_DEFS[scriptmodule_name] = module;
				MODULE_DEFS[scriptUtype] = module;
				...
				while(propertysize > 0)
				{
					...
					Logger.Debug("App::OnImportClientEntityDef:import property: name(%s), propertyUtype(%d), flags(%d), aliasID(%d), defaultValStr(%s), defaultVal(%s), utype(%d).",
				...
				while(methodsize > 0)
				{
					...
					let args: Array<DATATYPE_BASE> = new Array<DATATYPE_BASE>();

					Logger.Debug("App::OnImportClientEntityDef:import method:%s, utype(%d), aliasID(%d), args(%d).", name, methodUtype, aliasID, argssize);
					while(argssize > 0)
					{
						argssize--;
						let utype = stream.ReadUint16();
						args.push(datatypes[utype])
					}
					...
					if(module.script !== undefined)
					{
						...
						{
							Logger.Error("App::OnImportClientEntityDef:can not find def method:%s.", name);
						}
					}
				...
				while(base_methodsize > 0)
				{
					...
					Logger.Debug("App::OnImportClientEntityDef:import base method:%s, args(%d).", name, argssize);
					while(argssize > 0)
					{
						argssize--;
						let utype = stream.ReadUint16();
				...
				while(cell_methodsize > 0)
				{
					...
					Logger.Debug("App::OnImportClientEntityDef:import cell method:%s, args(%d).", name, argssize);
				...
				for(let name in module.methods)
				{
					...
					{
						Logger.Warning("Entity def %s::mehod(%s) no implement!", scriptmodule_name, name);
					}
				}
			}
		...
		private CreateAllDataTypeFromStream(stream: MemoryStream)
		{
			...
			Logger.Info("App::createDataTypeFromStreams: importAlias(size=" + aliasSize + ")!");
			...
			{
				//Logger.Debug("CreateAllDataTypeFromStream------------------->>>(key:%s), value:%s.", dataType, datatypes[dataType]);
		...
		{
			//Logger.Debug("CreateDataTypeFromStream------------------->>>");
			let utype = stream.ReadUint16();
			let name = stream.ReadString();
			...
			Logger.Info("App::CreateDataTypeFromStream: importAlias(" + utype + ":" + name + ":" + valname + ")!");
			...
			if(name === "FIXED_DICT")
			{
				...
				while(keysize-- > 0)
				{
					...
					//Logger.Debug("CreateDataTypeFromStream------------------->>>FIXED_DICT(valname:%s):keyutype:%d, keyname:%s", valname, keyutype, keyname);
			...
			else if(name === "ARRAY")
			{
				...
				//Logger.Debug("CreateDataTypeFromStream------------------->>>ARRAY(valname:%s), itemutype:%d.", valname, itemutype);
			}
			else
			{
		...
		Client_onUpdatePropertys(stream: MemoryStream)
		{
			...
			//Logger.Debug("Client_onUpdatePropertys------------------->>>eid:%s.", eid);
			this.OnUpdatePropertys(eid, stream);
		}
		...
		OnUpdatePropertys(eid: number, stream: MemoryStream)
		{
			...
			if(entity === undefined)
			{
				...
				{
					Logger.Error("App::OnUpdatePropertys: entity(%i) not found.", eid);
					return;
				}
			...
			let module: ScriptModule = MODULE_DEFS[entity.className];
			while(stream.Length() > 0)
			{
				...
				//Logger.Debug("App::OnUpdatePropertys: entity %s(id:%d, name:%s change oldval(%s) to val(%s), IsBase(%s),inited(%s).", 
		...
		{
			Logger.Info("App::Client_onCreatedProxies: uuid:(%s) eid(%d), entityType(%s)!", rndUUID.toString(), eid, entityType);
			...
			if(entity === undefined)
			{
				let scriptModule: ScriptModule = MODULE_DEFS[entityType];
				if(scriptModule === undefined)
				{
					Logger.Error("App::Client_onCreatedProxies:script(%s) is undefined.", entityType);
					return;
				}
		...
		OnRemoteMethodCall(eid: number, stream: MemoryStream)
		{
			...
			if(entity === undefined)
			{
				Logger.Error("App::Client_onRemoteMethodCall: entity(%d) not found!", eid);
				return;
			}
			...
			//Logger.Debug("App::OnRemoteMethodCall: methodUtype(%d), methodName(%s), use alias(%s).", 
			...
			{
				Logger.Error("App::Client_onRemoteMethodCall: entity(%d) not found method(%s)!", eid, defMethod.name);
			}
		}
		...
		{
			//Logger.Debug("Client_onRemoteMethodCallOptimized------------------->>>.");
			let eid = this.GetViewEntityIDFromStream(stream);
			this.OnRemoteMethodCall(eid, stream);
		}
		...
		{
			//Logger.Debug("Client_onEntityEnterWorld------------------->>>.");
			...
			let entityType = 0;
			let useScriptModuleAlias: boolean = Object.keys(MODULE_DEFS).length > 255;
			...
			if(entity === undefined)
			{
				...
				if(entityStream === undefined)
				{
					Logger.Error("KBEngine::Client_onEntityEnterWorld: entity(%d) not found!", eid);
					return;
				}

				let module: ScriptModule = MODULE_DEFS[entityType]
				if(module === undefined)
				{
					Logger.Error("KBEngine::Client_onEntityEnterWorld: not found module(" + entityType + ")!");
					return;
				}
		...
		Client_onEntityLeaveWorld(eid: number)
		{
			...
			if(entity === undefined)
			{
				Logger.Error("App::Client_onEntityLeaveWorld: entity(" + eid + ") not found!");
				return;
			}
			...
			else
			{
				...
				if(index !== -1)
				{
					...
					Event.Fire("onLoseControlledEntity", entity);
				}
		...
		Client_initSpaceData(stream: MemoryStream)
		{
			...
			Logger.Debug("KBEngine::Client_initSpaceData: spaceID(" + spaceID + "), size(" + Object.keys(this.spacedata).length + ")!");
		}

		Client_setSpaceData(spaceID: number, key: string, value: string)
		{
			Logger.Debug("KBEngine::Client_setSpaceData: spaceID(" + spaceID + "), key(" + key + "), value(" + value + ")!");
		...
		{
			Logger.Debug("KBEngine::Client_delSpaceData: spaceID(" + spaceID + "), key(" + key + ")");
		...
		Client_onEntityEnterSpace(stream: MemoryStream)
		{
			...
			if(entity === undefined)
			{
				Logger.Error("KBEngine::Client_onEntityEnterSpace: entity(" + eid + ") not found!");
				return;
			}
		...
		Client_onEntityLeaveSpace(eid: number)
		{
			...
			if(entity === undefined)
			{
				Logger.Error("KBEngine::Client_onEntityLeaveSpace: entity(" + eid + ") not found!");
				return;
			}
		...
		Player(): Player
		{
			return this.entities[this.entity_id] as Player;
		}

		Account(): Account
		{
			if(this.entity_type == "Account")
				return this.entities[this.entity_id] as Account;
			else
				throw new Error("No Account Entity");
		}
		...
		{
			Logger.Debug("KBEngine::addSpaceGeometryMapping: spaceID(" + spaceID + "), resPath(" + resPath + ")!");
		...
		{
			Logger.Error("App::Client_onKicked: failedcode(" + this.serverErrors[failedcode].name + ")!");
		...
		Client_onCreateAccountResult(stream: MemoryStream)
		{
			...
			if(retcode != 0)
			{
				Logger.Error("App::Client_onCreateAccountResult: " + this.userName + " create is failed! code=" + this.serverErrors[retcode].name + "!");
				return;
			}

			Logger.Info("App::Client_onCreateAccountResult: " + this.userName + " create is successfully!");
		}
		...
		Client_onReqAccountResetPasswordCB(failcode: number)
		{
			if(failcode != 0)
			{
				Logger.Error("KBEngine::Client_onReqAccountResetPasswordCB: " + this.userName + " is failed! code=" + failcode + "!");
				return;
			}

			Logger.Debug("KBEngine::Client_onReqAccountResetPasswordCB: " + this.userName + " is successfully!");
		}
		...
		Client_onReqAccountBindEmailCB(failcode: number)
		{
			if(failcode != 0)
			{
				Logger.Error("KBEngine::Client_onReqAccountBindEmailCB: " + this.userName + " is failed! code=" + failcode + "!");
				return;
			}

			Logger.Debug("KBEngine::Client_onReqAccountBindEmailCB: " + this.userName + " is successfully!");
		}
		...
		Client_onReqAccountNewPasswordCB(failcode: number)
		{
			if(failcode != 0)
			{
				Logger.Error("KBEngine::Client_onReqAccountNewPasswordCB: " + this.userName + " is failed! code=" + failcode + "!");
				return;
			}

			Logger.Debug("KBEngine::Client_onReqAccountNewPasswordCB: " + this.userName + " is successfully!");
		}

		Client_onEntityDestroyed(eid: number)
		{
			Logger.Debug("KBEngine::Client_onEntityDestroyed: entity(" + eid + ")");
			
			let entity = this.entities[eid];
			
			if(entity === undefined)
			{
				Logger.Error("KBEngine::Client_onEntityDestroyed: entity(" + eid + ") not found!");
				return;
			}
		...
		Client_onStreamDataRecv(stream: MemoryStream)
		{
			...
			let datas = stream.ReadBlob();
			Event.Fire("onStreamDataRecv", resID, datas);
		}
		...
		Client_onControlEntity(eid: number, isControlled: boolean)
		{
			...
			if (entity == undefined)
			{
				Logger.Error("KBEngine::Client_onControlEntity: entity(%d) not found!", eid);
				return;
			}
			...
			{
				Logger.Error("KBEngine::Client_onControlEntity: entity id = %d, is controlled = %s, error = %s", eid, isCont, e.toString());
			}
		}
		...
		UpdateVolatileData(entityID: number, x: number, y: number, z: number, yaw: number, pitch: number, roll: number, isOnGround: number)
		{
			...
			if(entity === undefined)
			{
				...
				// 则可以忽略这个错误, 因为cellapp可能一直在向baseapp发送同步消息， 当客户端重连上时未等
				// 服务端初始化步骤开始则收到同步信息, 此时这里就会出错。			
				Logger.Error("App::_updateVolatileData: entity(" + entityID + ") not found!");
				return;
			}
			...
			if(roll != KBE_FLT_MAX)
			{
				...
				entity.direction.x = Int8ToAngle(roll, false);
			}

			if(pitch != KBE_FLT_MAX)
			{
				changeDirection = true;
				entity.direction.y = Int8ToAngle(pitch, false);
			}
			
			if(yaw != KBE_FLT_MAX)
			{
				changeDirection = true;
				entity.direction.z = Int8ToAngle(yaw, false);
			}
		...
		{
			//Logger.Warning("Client_onUpdateBasePos---------->>>:x(%s),z(%s)..entityServerPos:x(%s),y(%s),z(%s).", x,z,this.entityServerPos.x,this.entityServerPos.y,this.entityServerPos.z);
		...
		{
			//Logger.Warning("Client_onUpdateBasePosXZ---------->>>:x(%s),z(%s)..entityServerPos:x(%s),y(%s),z(%s).", x,z,this.entityServerPos.x,this.entityServerPos.y,this.entityServerPos.z);
		...
		Client_onUpdateData(stream: MemoryStream)
		{
			...
			if(entity == undefined)
			{
				Logger.Error("App::Client_onUpdateData: entity(" + eid + ") not found!");
				return;
			}
		}
		...
		Client_onSetEntityPosAndDir(stream: MemoryStream)
		{
			...
			if(entity == undefined)
			{
				Logger.Error("App::Client_onSetEntityPosAndDir: entity(" + eid + ") not found!");
				return;
			}
