module Kbe
{
	...
	export class MemoryStream
	{
		...
		ReadString(): string
		{
			...
			while(true)
			{
				if(buf[index] != 0 )
				{
					...
					{
						throw(new Error("MemoryStream::ReadString overflow(>=) max length:" + this.buffer.byteLength));
					}
				}
				else
				{
		...
		WriteBlob(value: string|Uint8Array): void
		{
			...
			{
				Logger.Error("MemoryStream:WriteBlob:there is no space for size:%d", size + 4);
				return;
			}
		...
		WriteString(value: string): void
		{
			...
			{
				Logger.Error("MemoryStream:WriteString:there is no space for size:%d", value.length + 1);
				return;
			}
